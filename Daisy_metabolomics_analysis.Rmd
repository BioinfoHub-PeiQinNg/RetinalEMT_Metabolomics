...
ggplot(DF_cb_box_final, aes(x=Treatment, y=log(Counts), color = Treatment)) + 
  geom_boxplot() +theme_bw() + scale_colour_manual(values=c("#F8766D", "#7CAE00")) + facet_wrap(~) + theme(axis.text.x = element_text(angle = 45, hjust=1))
```
...
getExcluded(analysis.TNFa) #three compounds get excluded from the analysis
...
getExcluded(analysis.TGFb) #three compounds get excluded from the analysis
