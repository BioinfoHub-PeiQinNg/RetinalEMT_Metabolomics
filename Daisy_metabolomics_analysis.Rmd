...
metabolomics_metadata <- mutate_if(metabolomics_metadata, is.character, as.factor) %>%subset(select = -c(X)) %>%as.data.frame()
...
```
```{r}
DF <- assign_hierarchy(metabolomics_count_dataframe_all, keep_unknowns = TRUE, identifier = "KEGG" ) 

DF_sub <- DF %>% as.data.frame() %>% mutate_if(is.character, as.factor) %>% subset(select = -c(Class,Subclass_1,Subclass_2,Subclass_3,Subclass_4))
m
#DF_log <- log(DF[,3,50]) %>% as_tibble() %>% mutate(Metabolite = as.character(DF$Metabolites), KEGG = as.character(DF$KEGG)) %>% relocate(Metabolite, KEGG)
omu_summary(count_data = DF_sub , 
            metadata = metabolomics_metadata, 
            numerator = "Cells_Control", 
            denominator = "Cells_TNFa", 
            response_variable = "Metabolites", 
            Factor = "Treatment", 
            p_adjust = "BH", 
            test_type = "welch")
```
