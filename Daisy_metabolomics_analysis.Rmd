...
Generate code for heatmap (Cells : Control vs TNFa vs TGFb)
```{r}
...
p1 <- ggplot(DF_sub_ca, aes(x = Sample, y = Metabolite, fill = log(Counts))) +
...
DF_sub_2 <- DF_stats_ca %>% 
    tibble::as.tibble() %>% 
    dplyr::filter(log2FoldChange > 0.5 | log2FoldChange < -0.5 & 
...
p2 <- ggplot(DF_sub_ca_2, aes(x = Sample, y = Metabolite, fill = log(Counts))) +
  geom_tile(colour = " black") + 
    scale_fill_gradientn(colors = rev(hcl.colors(20, "RdYlBu"))) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
